...
```
{
  ...
  "scripts": {							// 自定义的脚本
    ...
    "coveralls": "npm run cover -- --report lcovonly && cat ./coverage/lcov.info | ./node_modules/.bin/coveralls"	// 第3方测试库，不用管
  },
  "author": "luo",		// 作者名字
  "license": "ISC",		// 开源协议
  ...
  "devDependencies": {										// 开发依赖包
    ...
    "babel-eslint": "^8.2.3",								// 让Eslint代码检测时支持一些新语法，比如Eslint本身不认识修饰器@（在.eslintrc.json配置了） 
    ...
    "eslint-plugin-react": "^7.7.0",						// Eslint插件 - 让Eslint支持检测React语法（JSX）（.eslintrc.json中配置的plugins参数）
    "express": "^4.16.3",									// Node.js框架 - 用于server.js中提供开发环境的服务
    "extract-text-webpack-plugin": "^4.0.0-beta.0",			// Webpack插件 - 用于提取项目中所有的CSS生成单独的.css文件（webpack.production.config.js用）
    "file-loader": "^1.1.11",								// Webpack解析器 - 解析各类文件时有用，图片、音频、视频等,处理它们的相对路径, url-loader依赖
    "happypack": "^5.0.0-beta.3",							// Webpack插件 - 多线程编译，速度更快，开发环境用
    "html-webpack-plugin": "^3.2.0",						// Webpack插件 - 最终打包时自动生成index.html，并自动配置index.html中的类容
    ...
    "preload-webpack-plugin": "^3.0.0-alpha.1",				// Webpack插件 - 在index.html中自动加入预编译的标签（选用，用处也不是很大）
    "prettier": "1.12.1",									// 代码自动格式化插工具
    "sass-loader": "^7.0.1",								// Webpack解析器 - 解析.sass/.scss文件，编译为.css
    "style-loader": "^0.20.3",								// Webpack解析器 - 用于提取重复的共用的css代码加入到index.html中的<style>标签里
    "url-loader": "^1.0.1",									// Webpack解析器 - 用于处理一些小图片，把它们编译为base64，同时也处理它们的相对路径
    "webpack": "^4.6.0",									// Webpack 核心
    "webpack-bundle-analyzer": "^2.11.1",					// Webpack插件 - 项目运行后自动弹出打包分析Tree图，项目中注释掉了，可自行打开
